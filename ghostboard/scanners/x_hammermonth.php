...
function fetchTickers() {
    $url = 'https://testapi.webepex.com/ghostboard/scanners/tickers/mhtickers.php';
...
$oneWeekBefore->modify('-1 month');
$oneWeekBeforeFormatted = $oneWeekBefore->format('Y-m-d');
    return [
        "intraday" => "https://financialmodelingprep.com/api/v3/historical-chart/4hour/{$symbol}?from={$oneWeekBeforeFormatted}&to={$currentDateFormatted}&apikey=wRyvg5z5qdd5lqbs4ProV1OMkOFFDUu3",
...
function fetchDataForTickers($tickers) {
    ...
    foreach ($tickers as $symbol) {
        ...
        $allData[] = [
            ...
            "open" => $intradayData[count($intradayData) - 1]["open"],
            "close" => $intradayData[0]["close"],
            "high" => max(array_column(array_slice($responses["intraday"], 0, count($intradayData) - 1), "high")),
            "low" => min(array_column(array_slice($responses["intraday"], 0, count($intradayData) - 1), "low")),
...
if (empty($allData)) {
    ...
    file_put_contents('hammermonth.json', json_encode($allData, JSON_PRETTY_PRINT));
