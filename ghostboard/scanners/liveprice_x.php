...
$marketDataApiUrl = 'https://financialmodelingprep.com/api/v3/stock/full/real-time-price?apikey=wRyvg5z5qdd5lqbs4ProV1OMkOFFDUu3';
...
foreach ($tickersArray as $ticker) {
    foreach ($marketData as $data) {
        if ($data['symbol'] === $ticker) {
            $tickerFmpLast[$ticker] = $data['fmpLast'];
...
$outputFile = 'ticker_fmp_last.json';
