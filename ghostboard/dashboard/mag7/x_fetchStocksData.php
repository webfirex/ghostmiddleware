...
function fetchTickers() {
    $url = 'https://testapi.webepex.com/ghostboard/dashboard/mag7/fetchTickers.php';
    $data = multiCurlGet([$url]);
    $data = $data[0]; // Only one URL, so get the first response

// Encode the result in the desired format
$datax = ["tickers" => $data];
...
function fetchDataForTickers($tickers) {
    ...
    foreach ($tickers as $symbol) {
        echo "Fetching data for ticker: {$symbol}";
        $urls = getApiUrls($symbol);
        $responses = multiCurlGet($urls);

        // Log the responses for debugging
        // echo("Responses for {$symbol}: " . print_r($responses, true));
       ...
       //  logMessage("Live price response for {$symbol}: " . print_r($livePrice, true));
        ...
        if (!$intradayData || !isset($intradayData[0]) || 
            ...
            echo "Incomplete data for {$symbol}. Responses: " . print_r($responses, true);
            continue;
        }
...
if (empty($allData)) {
    ...
    file_put_contents('stocksdata.json', json_encode($allData, JSON_PRETTY_PRINT));
    logMessage("Data has been stored in stocksdata.json.");
}

echo json_encode(["success" => "Data has been stored in stocksdata.json."]);
