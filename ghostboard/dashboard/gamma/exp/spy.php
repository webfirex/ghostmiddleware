...
$marketDataApiUrl = 'https://api.ghostboard.net/v1/exposure?symbol=spy';
...
// Check if 'result' key exists and process it
if (isset($marketData['result']) && is_array($marketData['result'])) {
    // Sort the keys numerically
    ksort($marketData['result'], SORT_NUMERIC);

    foreach ($marketData['result'] as $symbol => $values) {
        $transformedData["symbols"][] = (int)$symbol; // Add symbol to the array
        $transformedData["c"][] = $values["c"];       // Add 'c' value to the array
        $transformedData["p"][] = $values["p"];       // Add 'p' value to the array
    }
}

// Save the transformed data to JSON
$outputFile = 'spydata.json';
if (file_put_contents($outputFile, json_encode($transformedData, JSON_PRETTY_PRINT)) === false) {
